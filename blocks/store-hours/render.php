 ...
 */
function dokan_render_store_hours_block( array $attributes, string $content, WP_Block $block ): string {
	...
	$wrapper_attributes = get_block_wrapper_attributes(
		array(
			'class' => "dokan-store-hours dokan-store-hours-{$layout}",
		)
	);
	...
	<div <?php echo $wrapper_attributes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
		<?php if ( $show_current_status ) : ?>
			<div class="dokan-store-hours-status">
				<?php
		...
		<?php if ( 'detailed' === $layout ) : ?>
			<div class="dokan-store-hours-details">
				<h3><?php echo esc_html__( 'Weekly Store Timing', 'another-dokan-blocks' ); ?></h3>
				<ul class="dokan-store-hours-list">
					<?php foreach ( $dokan_days as $day_key => $day_label ) : ?>
						...
						?>
						<li class="dokan-store-hours-day <?php echo $today === $day_key ? 'today' : ''; ?>">
							<span class="day-name"><?php echo esc_html( $day_label ); ?></span>
							<span class="day-hours">
								<?php if ( $is_closed ) : ?>
									<span class="closed"><?php echo esc_html__( 'CLOSED', 'another-dokan-blocks' ); ?></span>
		...
		<?php else : ?>
			<div class="dokan-store-hours-compact">
				<p>
					<?php
