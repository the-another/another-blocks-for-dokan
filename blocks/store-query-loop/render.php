...
function dokan_render_store_list_block( array $attributes, string $content, WP_Block $block ): string {
	...
	$wrapper_attributes = get_block_wrapper_attributes(
		array(
			'class' => "dokan-store-list dokan-store-list-{$display_layout} dokan-store-list-columns-{$columns}",
		)
	);
	...
	if ( ! empty( $sellers ) ) {
		...
		$grid_classes = 'dokan-store-list-wrap';
		if ( 'grid' === $display_layout ) {
			$grid_classes .= ' dokan-store-list-grid dokan-store-list-columns-' . absint( $columns );
		} elseif ( 'list' === $display_layout ) {
			$grid_classes .= ' dokan-store-list-list';
		}

		// Separate template blocks (store-card) from query-level blocks (search, pagination).
		...
		if ( ! empty( $block->inner_blocks ) ) {
			foreach ( $block->inner_blocks as $inner_block ) {
				...
				} elseif ( 'the-another/dokan-store-card' === $inner_block->name ) {
		...
		<div <?php echo $wrapper_attributes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
			...
			?>
			<ul class="dokan-seller-wrap <?php echo esc_attr( $grid_classes ); ?>">
				<?php
				foreach ( $sellers as $seller ) {
					...
					<li class="dokan-single-seller">
						<?php
						// Render template blocks (store-card) with vendor context.
						if ( ! empty( $template_blocks ) ) {
							...
							// Fallback to default template if no store-card blocks.
							?>
							<div class="dokan-store-card">
								<?php
								if ( function_exists( 'dokan_render_store_avatar_block' ) ) {
								...
								if ( function_exists( 'dokan_render_store_name_block' ) ) {
			...
			if ( ! $pagination_rendered && $total_pages > 1 ) :
				?>
				<nav class="dokan-store-list-pagination">
					<?php
					echo wp_kses_post(
						paginate_links(
							array(
		...
		<div <?php echo $wrapper_attributes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
			...
			?>
			<p class="dokan-store-list-empty">
