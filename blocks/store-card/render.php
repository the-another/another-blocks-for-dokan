<?php
/**
 * Store card block render function.
 *
...
function dokan_render_store_card_block( array $attributes, string $content, WP_Block $block ): string {
	...
	$wrapper_classes = array( 'dokan-store-card' );
	...
	if ( ! $vendor_id || ! $vendor_data ) {
		...
		if ( is_admin() || ( defined( 'REST_REQUEST' ) && REST_REQUEST ) ) {
			...
			<div <?php echo $wrapper_attributes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?><?php echo $style_attr; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
				<div class="dokan-store-card-placeholder">
					<?php if ( ! $vendor_id ) : ?>
						<p class="store-placeholder-notice" style="padding: 1rem; background: #f0f0f0; border-radius: 4px; font-size: 0.875rem; color: #666;">
							<?php echo esc_html__( 'Please enter a valid Vendor ID in the block settings to display a store card.', 'another-dokan-blocks' ); ?>
						</p>
					<?php else : ?>
						<p class="store-placeholder-notice" style="padding: 1rem; background: #fff3cd; border-radius: 4px; font-size: 0.875rem; color: #856404;">
