...
function dokan_render_store_tabs_block( array $attributes, string $content, WP_Block $block ): string {
	...
	<div <?php echo $wrapper_attributes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
		<ul class="dokan-list-inline dokan-store-tabs-list" role="tablist">
			<?php foreach ( $store_tabs as $key => $tab ) : ?>
				<?php if ( ! empty( $tab['url'] ) ) : ?>
					<?php
					$is_active  = dokan_blocks_is_tab_active( $key, $tab['url'] );
					$tab_class  = $is_active ? 'dokan-store-tab-item active' : 'dokan-store-tab-item';
