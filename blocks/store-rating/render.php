 ...
 */
function dokan_render_store_rating_block( array $attributes, string $content, WP_Block $block ): string {
	...
	if ( empty( $vendor ) || empty( $vendor['id'] ) ) {
		return '<div class="dokan-store-rating">★★★★★ (0)</div>';
	}
	...
	<div <?php echo $wrapper_attributes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
		...
		if ( function_exists( 'dokan_generate_ratings' ) ) {
			...
			<div class="star-rating" role="img" aria-label="<?php echo esc_attr( sprintf( __( 'Rated %s out of 5', 'another-dokan-blocks' ), $rating ) ); ?>">
