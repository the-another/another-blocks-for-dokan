<?php
/**
 * Store Logo block render function.
 *
 ...
 */
function dokan_render_store_avatar_block( array $attributes, string $content, WP_Block $block ): string {
	...
	if ( empty( $vendor ) || empty( $vendor['id'] ) ) {
		return '<div class="dokan-store-avatar"><img src="' . esc_url( get_avatar_url( 0 ) ) . '" alt="" /></div>';
	}
	...
	$wrapper_classes = array( 'dokan-store-avatar' );
	if ( ! empty( $align ) ) {
	...
	<div <?php echo $wrapper_attributes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?><?php echo $wrapper_style_attr; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
		<?php if ( $is_link && ! empty( $shop_url ) ) : ?>
			<a href="<?php echo esc_url( $shop_url ); ?>" class="dokan-store-avatar-link">
				<img
					src="<?php echo esc_url( $avatar_url ); ?>"
					alt="<?php echo esc_attr( $shop_name ); ?>"
					class="dokan-store-avatar-image"
