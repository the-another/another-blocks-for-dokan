...
// Default template for store cards - includes search at the beginning and pagination at the end
const TEMPLATE = [
	[ 'the-another/dokan-store-search', { enableSearch: true, enableSortBy: true } ],
	[ 'the-another/dokan-store-card', {
		layout: { type: 'flex', orientation: 'vertical', justifyContent: 'center' },
	}, [
		[ 'the-another/dokan-store-avatar', { width: '10rem', height: '10rem' } ],
		[ 'core/group', { 
	...
	[ 'the-another/dokan-store-query-pagination', { paginationArrow: 'arrow', showLabel: true } ],
];

// Allowed blocks inside store query loop (top level)
const ALLOWED_BLOCKS = [
	'the-another/dokan-store-card',
	'the-another/dokan-store-query-pagination',
...
function Edit( { attributes, setAttributes } ) {
	...
	const blockProps = useBlockProps( {
		className: `dokan-store-list dokan-store-list-${ displayLayout }`,
	} );
