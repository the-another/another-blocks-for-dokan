...
function Edit( { attributes, setAttributes, context } ) {
	...
	const blockProps = useBlockProps( {
		className: `dokan-store-hours dokan-store-hours-${ layout }`,
	} );
	...
	return (
		<>
			...
			<div { ...blockProps }>
				{ ! storeTimeEnabled ? (
					<div className="dokan-store-hours-disabled">
					...
					<>
						{ showCurrentStatus && (
							<div className="dokan-store-hours-status">
								{ isOpen ? (
						...
						{ layout === 'detailed' ? (
							<div className="dokan-store-hours-details">
								<h4>{ __( 'Weekly Store Timing', 'dokan-blocks' ) }</h4>
								<ul className="dokan-store-hours-list">
									{ Object.entries( DAYS ).map( ( [ dayKey, dayLabel ] ) => {
										...
										return (
											<li 
												key={ dayKey } 
												className={ `dokan-store-hours-day ${ isToday ? 'today' : '' }` }
											>
							...
							<div className="dokan-store-hours-compact">
								<span className="dashicons dashicons-clock"></span>
								{ isOpen ? (
									<span>
