...
function dokan_render_store_search_block( array $attributes, string $content, WP_Block $block ): string {
	...
	$wrapper_attributes = get_block_wrapper_attributes(
		array(
			'class' => 'dokan-store-search',
		)
	);

	// Generate button classes and styles (reusable for all buttons).
	$button_classes = array( 'dokan-btn', 'dokan-btn-theme' );
	...
	<div <?php echo $wrapper_attributes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
		...
		<div class="dokan-store-listing-filter-wrap">
			<div class="dokan-store-filter-row">
				<div class="dokan-store-filter-row-inner">
					<div class="dokan-store-filter-left">
						<?php
					...
					<?php if ( $enable_search ) : ?>
						<div class="dokan-store-filter-right-item">
							<div class="item">
								...
								<button type="button" class="dokan-store-list-filter-button <?php echo esc_attr( $button_class_string ); ?>" style="<?php echo ! empty( $button_style_string ) ? esc_attr( $button_style_string ) : ''; ?>" aria-expanded="false" aria-controls="dokan-store-listing-filter-form-wrap">
									<span class="dokan-btn-text"><?php echo esc_html__( 'Filter', 'another-dokan-blocks' ); ?></span>
								</button>
							</div>
						</div>
					<?php endif; ?>
				...
				<?php if ( $enable_sort_by ) : ?>
					<div class="dokan-store-filter-row-inner dokan-store-filter-row-sort">
		...
		<?php if ( $enable_search ) : ?>
			...
			?>
			<form role="store-list-filter" method="get" name="dokan_store_lists_filter_form" id="dokan-store-listing-filter-form-wrap" class="dokan-store-search-filter-form" style="<?php echo $has_active_filters ? 'display: block;' : 'display: none;'; ?>">
				...
				<div class="dokan-store-search-filter-row">
					<div class="store-search grid-item">
						<input type="search" 
							class="store-search-input dokan-store-search-input" 
					...
					<div class="apply-filter">
						<button id="cancel-filter-btn" type="button" class="<?php echo esc_attr( $button_class_string ); ?>" style="<?php echo ! empty( $button_style_string ) ? esc_attr( $button_style_string ) : ''; ?>">
							<span class="dokan-btn-text"><?php echo esc_html__( 'Cancel', 'another-dokan-blocks' ); ?></span>
						</button>
						<button id="apply-filter-btn" type="submit" class="<?php echo esc_attr( $button_class_string ); ?>" style="<?php echo ! empty( $button_style_string ) ? esc_attr( $button_style_string ) : ''; ?>">
							<span class="dokan-btn-text"><?php echo esc_html( $button_text ); ?></span>
						</button>
					</div>
				</div>

				<?php if ( $enable_location_filter || $enable_rating_filter || $enable_category_filter ) : ?>
					<div class="dokan-store-advanced-filters">
						<?php if ( $enable_location_filter ) : ?>
							<div class="dokan-store-filter-field">
								<label><?php echo esc_html__( 'Location:', 'another-dokan-blocks' ); ?></label>
								<select name="dokan_store_location" class="dokan-store-filter-select">
						...
						<?php if ( $enable_rating_filter ) : ?>
							<div class="dokan-store-filter-field">
								...
								<select name="dokan_store_rating" class="dokan-store-filter-select">
							...
							<div class="dokan-store-filter-field">
								...
								wp_dropdown_categories(
									array(
										...
										'value_field'      => 'slug',
										'class'            => 'dokan-store-filter-select',
									)
								);
								?>
							</div>
	...
	<script>
	(function() {
		var filterButton = document.querySelector('.dokan-store-list-filter-button');
		var filterForm = document.getElementById('dokan-store-listing-filter-form-wrap');
