 ...
 */
function dokan_render_store_status_block( array $attributes, string $content, WP_Block $block ): string {
	...
	if ( empty( $vendor ) || empty( $vendor['id'] ) ) {
		return '<span class="dokan-store-status dokan-store-open">' . esc_html__( 'Open Now', 'another-dokan-blocks' ) . '</span>';
	}
	...
	$status_class       = $is_store_open ? 'dokan-store-open' : 'dokan-store-closed';
	$wrapper_attributes = get_block_wrapper_attributes(
		array(
			'class' => "dokan-store-status {$status_class}",
		)
	);
