 ...
 */
function dokan_render_store_header_block( array $attributes, string $content, WP_Block $block ): string {
	// Capability check - allow anyone to view store headers.
	...
	$wrapper_attributes = get_block_wrapper_attributes(
		array(
			'class' => "dokan-store-header dokan-store-header-{$layout}",
		)
	);
					...
					<?php if ( $show_store_hours ) : ?>
						<li class="dokan-store-hours">
							...
							if ( \The_Another\Plugin\Blocks_Dokan\Renderers\Vendor_Renderer::is_store_open( $vendor_id ) ) {
								echo '<span class="store-open">' . esc_html__( 'Store Open', 'another-dokan-blocks' ) . '</span>';
							} else {
								echo '<span class="store-closed">' . esc_html__( 'Store Closed', 'another-dokan-blocks' ) . '</span>';
							}
							?>
						</li>
					<?php endif; ?>
